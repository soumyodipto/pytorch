...
def run_test(
    ...
    if test_module not in PYTEST_INCOMPATIBLE:
        unittest_args.extend(["--use-pytest", '-vv', '-x', '--reruns=2', '-rfEX', "-k=not _linalg_cholesky"])
        if options.large_test:
            which_shard, num_shards = options.shard or (1, 1)
            unittest_args.extend([f'--shard-id={which_shard - 1}', f'--num-shards={num_shards}'])
...
def get_selected_tests(options, test_times: Optional[Dict[str, float]]):
    ...
    if options.shard:
        ...
        if test_times is None:
            print("Using default sharding plan")
...
def main():
    ...
    try:
        for test in selected_tests_parallel:
            ...
            options_clone.large_test = test_times is not None and test_times.get(test, 0) > LARGE_TEST
